##### ICS Practice IGNORE
##### This is a script for Seshadri Lab members to practice flow cytometry analysis in R
...
# New Session - Load the saved Gating Set
gs_b1 <- load_gs(here::here("out/GatingSets/20210826_HAARVI_ICS_GatingSet_B1"))

# Let's extract CD4 counts for all samples and plot the distribution stratified by PTID or STIM
nodelist <- gs_get_pop_paths(gs_b1, path = "full")           # Get the names of nodes in the gating tree
nodelist
cd4_path <- nodelist[7]                                      # Choose the CD4 node and extract count
cd4_count <- gs_pop_get_stats(gs_b1, nodes = cd4_path)
head(cd4_count)
pd <- pData(gs_b1)                                           # Extract phenotype data
head(pd)
pd <- tibble::rownames_to_column(pd,"sample")                # We need 'sample' to merge, but it is not in the data frame, 
cd4_count_sample <- merge(cd4_count, pd, by="sample")        # so we add as a column and them merge
head(cd4_count_sample)
names(cd4_count_sample)[7] <- "SAMPLE_ID"                    # We need to do this, so we can stratify boxplot
with(cd4_count_sample, boxplot(count~STIM))                  # Plot CD4 counts stratified by STIM
with(cd4_count_sample, boxplot(count~SAMPLE_ID))             # Plot CD4 counts stratified by PTID
